<template>
  <v-container id="login" class="fill-height" tag="section">
        ...
        <base-material-card color="gray" light max-width="100%" width="400" class="px-5 py-3">
          ...
          <v-card-text class="text-center">
            <v-form ref="form" v-model="valid" lazy-validation>
              <v-text-field v-model="user.email" color="secondary" label="Correo electrónico" prepend-icon="mdi-email"
                required :rules="[rules.required]" />

              <!-- :rules="[rules.required,rules.emailRules]" -->
              <v-text-field v-model="user.password" :append-icon="show1 ? 'mdi-eye' : ' mdi-eye-off'" required
                :type="show1 ? 'text' : 'password'" name="input-10-1" label="Contraseña" prepend-icon="mdi-lock-outline"
                hint="minimo 8 caracteres" counter :rules="[rules.required, rules.min]" @click:append="show1 = !show1" />
              <br>
              <pages-btn large dark color="success" depressed class="v-btn--text white--text font-weight-bold"
                style="text-transform: capitalize;" @click="submit">
                Iniciar sesión
              </pages-btn>
            </v-form>
            <!-- <v-col cols="12">
    ...
    <v-dialog v-model="dialog" max-width="290" class="dialog-error">
